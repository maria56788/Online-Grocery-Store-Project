...
function displayCart(){
     cartdata="<table><tr><th>Product Name</th><th>Quantity</th><th>Price</th>Total</th></tr>";
     total=0;

     for(i=0;i<inames.length;i++){
        total+=iqtyp[i]*iprice[i];
        cartdata+="<tr><td>"+inames[i]+"</td><td>"+
        iqtyp[i]+"</td><td>"+iprice[i]+"</td><td>"+
        iqtyp[i]*iprice[i]+"</td><td><button onclick='delElement(" + i + ")'></button>Delete</td></tr>"
     }
